...
# Adicione estas linhas logo no início do script, fora de labels
...
# --- POSIÇÕES CORPO INTEIRO (Originais) ---
transform pos_leo:
...
label conversa_maya:
    ...
    # Verifica se há mensagem de história (Bolinha Vermelha)
    if persistent.luisa_derrotada and nova_msg_maya:
        show maya normal at maya_busto
        m "Theo! Eu não acredito! A Luísa me contou que você venceu ela!"
        $ nova_msg_maya = False
    
    # Caso contrário, manda uma resposta aleatória da sua lista
    else:
        $ resposta = random.choice(respostas_maya)
        show maya normal at maya_busto
        m "[resposta]"
...
label conversa_leo:
    ...
    if persistent.luisa_derrotada and nova_msg_leo:
        show leo feliz at leo_busto
        l "Cara, bater a Luísa no Desafio de Abertura? Você é monstro!"
        $ nova_msg_leo = False
    else:
        $ resposta = random.choice(respostas_leo)
...
label conversa_luisa:
    ...
    if persistent.luisa_derrotada and nova_msg_luisa:
        ...
        "Mensagem de Luísa: 'Analisei nossa partida. Você foi preciso no endgame.'"
        $ nova_msg_luisa = False
    else:
        $ resposta = random.choice(respostas_luisa)
        show luisa normal at luisa_busto
        # Como é mensagem de texto, podemos exibir sem o "show luisa"
        "Luísa enviou uma mensagem: '[resposta]'"
